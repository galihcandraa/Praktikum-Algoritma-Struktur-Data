...
public class BinaryTree10 {
    ...
    public void searchByName(String nama) {
        searchByName(root, nama);
    }

    private void searchByName(Node10 node, String nama) {
        if (node == null) {
            return;
        }
        ...
        searchByName(node.left, nama);
        searchByName(node.right, nama);
    }

    public void searchNPrintByClass(String kelas) {
        searchNPrintByClass(root, kelas);
    }

    private void searchNPrintByClass(Node10 node, String kelas) {
        if (node == null) {
            return;
        }

        searchNPrintByClass(node.left, kelas);

        if (node.mahasiswa.kelas.equalsIgnoreCase(kelas)) {
            node.mahasiswa.tampilInformasi();
        }

        searchNPrintByClass(node.right, kelas);
    }
    ...
    private int hitungTop = 0;

    public void cari3IPKTertinggi() {
        hitungTop = 0;
        cari3IPKTertinggi(root);
    }

    private void cari3IPKTertinggi(Node10 node) {
        if (node == null || hitungTop >= 3) {
            return;
        }

        cari3IPKTertinggi(node.right);
